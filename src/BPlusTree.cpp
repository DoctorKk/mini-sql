...
{
    CNode<KEY_TYPE>::m_Type = NODE_TYPE_INTERNAL;
...
bool CInternalNode<KEY_TYPE>::Insert(KEY_TYPE value, CNode<KEY_TYPE>* pNode)
{
    int i;
    // 如果中间结点已满，直接返回失败
    if (CNode<KEY_TYPE>::GetCount() >= MAXNUM_KEY)
    {
        return false;
    }
    ...
    // 找到要插入键的位置
    for (i = 0; (value > m_Keys[i]) && ( i < CNode<KEY_TYPE>::m_Count); i++)
    {
    }

    // 当前位置及其后面的键依次后移，空出当前位置
    for (j = CNode<KEY_TYPE>::m_Count; j > i; j--)
    {
        m_Keys[j] = m_Keys[j - 1];
    }

    // 当前位置及其后面的指针依次后移
    for (j = CNode<KEY_TYPE>::m_Count + 1; j > i + 1; j--)
    {
...
{
    int i,j,k;
    for (i = 0; (key >= m_Keys[i]) && (i < CNode<KEY_TYPE>::m_Count); i++)
    {
    }

    for (j = i - 1; j < CNode<KEY_TYPE>::m_Count - 1; j++)
    {
        m_Keys[j] = m_Keys[j + 1];
    }
    //m_Keys[j] = INVALID;
...
*/
template <class KEY_TYPE>
KEY_TYPE CInternalNode<KEY_TYPE>::Split(CInternalNode* pNode, KEY_TYPE key)  //key是新插入的值，pNode是分裂结点
{
    int i = 0, j = 0;
...
bool CInternalNode<KEY_TYPE>::Combine(CNode<KEY_TYPE>* pNode)
{
    ...
    m_Keys[CNode<KEY_TYPE>::m_Count] = NewKey;
    CNode<KEY_TYPE>::m_Count++;
    m_Pointers[CNode<KEY_TYPE>::m_Count] = pNode->GetPointer(1);   //疑问：感觉应该为m_Pointers[CNode<KEY_TYPE>::m_Count+1] = pNode->GetPointer(1);

    for (int i = 1; i <= pNode->GetCount(); i++)
    {
        m_Keys[CNode<KEY_TYPE>::m_Count] = pNode->GetElement(i);
        CNode<KEY_TYPE>::m_Count++;
        m_Pointers[CNode<KEY_TYPE>::m_Count] = pNode->GetPointer(i+1);
    }
...
bool CInternalNode<KEY_TYPE>::MoveOneElement(CNode<KEY_TYPE>* pNode)
{
    ...
    {
        // 先腾出位置
        for (i = CNode<KEY_TYPE>::m_Count; i > 0; i--)
        {
            m_Keys[i] = m_Keys[i -1];
        }
        for (j = CNode<KEY_TYPE>::m_Count + 1; j > 0; j--)
        {
            m_Pointers[j] = m_Pointers[j -1];
        }
    ...
    else    // 兄弟结点在本结点右边
    {
        ...
        m_Keys[CNode<KEY_TYPE>::m_Count] = pNode->GetPointer(1)->GetElement(1);
        // 最后一个子结点为兄弟结点的第一个子结点
        m_Pointers[CNode<KEY_TYPE>::m_Count + 1] = pNode->GetPointer(1);
...
{
    CNode<KEY_TYPE>::m_Type = NODE_TYPE_LEAF;
...
// 在叶子结点中插入数据
template <class KEY_TYPE>
bool CLeafNode<KEY_TYPE>::Insert(pair<int, KEY_TYPE> value)
{
    int i,j;
    // 如果叶子结点已满，直接返回失败
    if (CNode<KEY_TYPE>::GetCount() >= MAXNUM_DATA)
    {
        return false;
    }

    // 找到要插入数据的位置
    for (i = 0; (value.second > m_Datas[i].second) && ( i < CNode<KEY_TYPE>::m_Count); i++)
    {
    }
    ...
    for (j = CNode<KEY_TYPE>::m_Count; j > i; j--)
    {
        m_Datas[j].second = m_Datas[j - 1].second;
        m_Datas[j].first = m_Datas[j - 1].first;
    }

    // 把数据存入当前位置
    m_Datas[i].second = value.second;
    m_Datas[i].first = value.first;
...
bool CLeafNode<KEY_TYPE>::Delete(KEY_TYPE value)
{
    int i,j;
    bool found = false;
    for (i = 0; i < CNode<KEY_TYPE>::m_Count; i++)
    {
        if (value == m_Datas[i].second)
        {
            found = true;
            break;
        }
    ...
    // 后面的数据依次前移
    for (j = i; j < CNode<KEY_TYPE>::m_Count - 1; j++)
    {
        m_Datas[j].second = m_Datas[j + 1].second;
        m_Datas[j].first = m_Datas[j + 1].first;
    }
...
bool CLeafNode<KEY_TYPE>::Combine(CNode<KEY_TYPE>* pNode)
{
    ...
    for (int i = 1; i <= pNode->GetCount(); i++)
    {
        this->Insert(make_pair(pNode->GetElementOffset(i), pNode->GetElement(i)));
    }
...
int BPlusTree<KEY_TYPE>::Search(KEY_TYPE data)
{
    ...
    for (i = 1; (i <= pNode->GetCount()); i++)
    {
        ...
        {
            answer = pNode->GetElementOffset(i) ;
        }
    }
...
bool BPlusTree<KEY_TYPE>::Insert(pair<int, KEY_TYPE> data)  //
{
    ...
    // 查找理想的叶子结点
    CLeafNode<KEY_TYPE>* pOldNode = SearchLeafNode(data.second);
    ...
    if (NULL == pFather)
    {
        CInternalNode<KEY_TYPE>* pNode1 = new CInternalNode<KEY_TYPE>;
...
bool BPlusTree<KEY_TYPE>::Delete(KEY_TYPE data)
{
    ...
    // 兄弟结点填充度>50%，对应情况2A
    pair<int, KEY_TYPE> NewData;
    NewData.second;
    if (pBrother->GetCount() > ORDER_V)
    {
        if (FLAG_LEFT == flag)    // 兄弟在左边，移最后一个数据过来
        {
            NewData.second = pBrother->GetElement(pBrother->GetCount());
            NewData.first = pBrother->GetElementOffset(pBrother->GetCount());
        }
        else    // 兄弟在右边，移第一个数据过来
        {
            NewData.second = pBrother->GetElement(1);
            NewData.first = pBrother->GetElementOffset(1);
        }

        pOldNode->Insert(NewData);
        pBrother->Delete(NewData.second);
...
bool BPlusTree<KEY_TYPE>::InsertInternalNode(CInternalNode<KEY_TYPE>* pNode, KEY_TYPE key, CNode<KEY_TYPE>* pRightSon)
{
    ...
    CInternalNode<KEY_TYPE>* pBrother = new CInternalNode<KEY_TYPE>;  //C++中new 类名表示分配一个类需要的内存空间，并返回其首地址；
    KEY_TYPE NewKey;
    // 分裂本结点
